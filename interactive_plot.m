...
mpage.marker_bt7 = uicontrol('style','text',...
    ...
    'string','*Frame ID*',...
    'BackgroundColor', 'w');
%% save marker file as txt
mpage.marker_bt8 = uicontrol('style','push',...
    'units','normalized',...
    'position',[5/1200 300/600 60/1200 20/600],...
    'fontsize',7,...
    'string','Save As',...
    'callback',{@save_marker_file, mpage});
...
mpage.video_bt7 = uicontrol('style','push',...
    'units','normalized',...
    'position',[1100/1200 340/600 60/1200 20/600],...
    'fontsize',7,...
    'string','Save As',...
    'callback',{@save_video_frame, mpage});
...
function save_marker_file(~,~,~)
global marker_fid;
global pose_st;

marker_st=pose_st.xyz_all;
marker_xyz=marker_st{3}(:,2);

current_marker=[double(marker_fid), marker_st{2}(marker_fid)];
for i=1:37 
current_marker=[current_marker,marker_xyz{i}(marker_fid,:)];
end
[file_name, file_path]=uiputfile('*.txt','Save as TXT',['marker_frame_',...
    num2str(marker_fid),'.txt']);
dlmwrite(fullfile(file_path, file_name),current_marker);
end
...
function save_video_frame(~,~,mpage)
global frame_id;
current_frame=getimage(mpage.image_axes);
[file_name, file_path]=uiputfile('*.png','Save as PNG',['video_frame_',...
    num2str(frame_id),'.png']);
imwrite(current_frame, fullfile(file_path, file_name));
end
%% other functions
